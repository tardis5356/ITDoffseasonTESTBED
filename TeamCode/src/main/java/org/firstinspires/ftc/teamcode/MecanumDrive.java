...
public final class MecanumDrive {
    ...
    public Localizer localizer; // TODO remove final
    public Pose2d pose;
...
//        leftBack.setDirection(DcMotor.Direction.REVERSE);
    ...
    public PoseVelocity2d updatePoseEstimate() {
        ...
        if(telemetry != null){
            telemetry.addLine("Pose" + pose.position);
            telemetry.addLine("HEading" + Math.toDegrees(pose.heading.log()));
            telemetry.update();
        }
...
//                0.25, 0.1
    ...
    public  void driveFieldCentric(double xPow, double yPow, double rotPow){

        if(Math.abs(xPow)<.1&&Math.abs(yPow)<.1){setDrivePowers(new PoseVelocity2d(new Vector2d(0,0),rotPow));}
        else{
            double targetTheta = Math.atan2(Math.toRadians(yPow),Math.toRadians(xPow));
            double robotTheta = getHeading() + Math.PI/2;
            double diffTheta = Math.toDegrees(targetTheta)-Math.toDegrees(robotTheta);
            //diffTheta += Math.PI/2;


            xPow = Math.cos(Math.toRadians(diffTheta));
            yPow = Math.sin(Math.toRadians(diffTheta));
    ...
    public double getHeading(){
        return imu.getRobotYawPitchRollAngles().getYaw(AngleUnit.RADIANS)- Math.toRadians(offsetHeading) + Math.toRadians(initialHeading);
    }
    public void resetHeading(){
        offsetHeading = imu.getRobotYawPitchRollAngles().getYaw(AngleUnit.DEGREES);
    }
